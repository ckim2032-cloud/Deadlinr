...
function deleteClassFromPrompt() {
    ...
    if (name) {
        ...
        if (index > -1) {
            ...
            masterData.forEach(t => { if (t.class === cleanName) t.class = classes[0]; });
            save();
            renderAll();
        }
    }
...
function addRow() {
    masterData.push({ name: "", class: classes[0], status: "todo", date: "", priority: "med" });
    save();
    renderAll();
}
...
function animateWeek(direction, newStart) {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => { calendarAnimating = false; }, 240);
    }, 220);
}
